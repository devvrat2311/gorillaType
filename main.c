...
int main() {
  ...
  char *p = &displayString[0]; //to be noted here that p is still just a character pointer but the way we
...
void display() {
  ...
  if(state == 0) {
    ...
    printf(ANSI_COLOR_RED"\n\n \t\t\t%.*s\n" ANSI_COLOR_RESET, len - 1, testString);
...
int handleTyping(char ch, char** p) { //in this function p is a pointer to the pointer which points to the carets position in the string
